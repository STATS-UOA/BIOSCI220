...
## plot
simple <- ggplot(data = penguins_nafree, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point() + ylab("Bill depth (mm)") +
  xlab("Bill length (mm)") + theme_classic() +
...
## plot
add <- ggplot(data = penguins_nafree, aes(y = bill_depth_mm, x = bill_length_mm, color = species)) +
  geom_point() + ylab("Bill depth (mm)") +
  xlab("Bill length (mm)") + theme_classic()  +
  geom_line(aes(y = pred_vals))
add
...
## plot
int <- ggplot(data = penguins_nafree, aes(y = bill_depth_mm, x = bill_length_mm, color = species)) +
  geom_point() + ylab("Bill depth (mm)") +
  xlab("Bill length (mm)") + theme_classic()  +
  geom_line(aes(y = pred_vals))
int
...
library(patchwork)
null_model <- ggplot(penguins_nafree,aes(x = 1:length(bill_depth_mm), y = bill_depth_mm)) +
  geom_point() + xlab("Index") + ylab("Bill depth (mm)") + theme_classic() + geom_hline(yintercept = mean(penguins_nafree$bill_depth_mm))

(null_model | simple)/(add | int) + plot_annotation(tag_levels = 'A')
```

`r countdown::countdown(minutes = 1, seconds = 0, top = 0)`
...
```{r, echo = FALSE}
equatiomatic::extract_eq(slm_sp,wrap = TRUE, terms_per_line = 1,use_coefs = TRUE)
```
...
> To call in the statistician after the experiment is done may be no more than asking him to perform a postmortem examination... he may be able to say what the experiment died of `r tufte::quote_footer('--- R.A. Fisher, Indian Statistical Congress, Sankhya, ca 1938')`

---

class: inverse, center, middle

> To call in the statistician after the experiment is done may be no more than asking ~~him~~ them to perform a postmortem examination... ~~he~~ they may be able to say what the experiment died of `r tufte::quote_footer('--- R.A. Fisher, Indian Statistical Congress, Sankhya, ca 1938')`
...
+ **Explain** the aims and motivation behind 
...
**AIM**: find the best low-dimensional representation of the variation in a multivariate data set
-- 

+ PCA
  + reduces the dimensionality of the data (number of variables) by creating new uncorrelated variables while minimizing loss of information on the original variables
...
### PCA in `R`
 ...
 + `flipper_length_mm`, `body_mass_g`, and `bill_length_mm` are relatively strongly (positively) related to one another

--

+ let's call this penguin *bigness*

--

+ Is there a way we could reduce these three variables to represent the *bigness* of a penguin.

---
### PCA in `R`

**Scale your data**: often the data variables are on very different scales (e.g., grams and millimeters) we should first make sure that each variable is on the same **scale**
...
+ **PC scores:**  the **new uncorrelated variables** 

+ **Eigenvalues:** represent the variance explained by each PC

+ **Variable loadings** (eigenvectors): can be thought of as the correlation between the PC and the original variable
...
+ `PC1` explains $\sim$ 68% of the total variance, which means that just over half of the information in the dataset 
 (4 variables) can be encapsulated by just that one Principal Component. 
...
---
**[slido.com](slido.com) code #biosci220: What do the PCs represent (roughly)**
...
+ **Variable loadings** (eigenvectors): `pca$rotation`
...
**AIM:** categorise data (find groups) in unlabeled data

--

+ k-means: number of groups represented by the variable $k$

--

+  It's all about variation, and the idea of minimizing it, again!
---
...
+ $k$ objects from the dataset are selected at random and *set* as the initial cluster centers 

--

+ Each observation is assigned to their closest centroid

--

+ For each of the $k$ clusters the cluster centroid is then updated 
--
...
**ClusterDucks; in Sydney, the ducks have their own** [fashion show](https://mymodernmet.com/australian-pied-piper-duck-show/)

<img src = "https://github.com/cmjt/statbiscuits/blob/master/swots/images/blue_flowers-2.jpg?raw=true" height="50%">
...
+ In total we have `r length(duck_rgbs)` different images of `r length(table(stringr::str_match(names(duck_rgbs),"(.*?)-")[,2]))` different outfits.

+ Each image is summarised by  by the average R, G, and B value respectively.

---
### RGB ClusterDucks

```{r, echo = FALSE, fig.cap = "3D scatterplot of the average RGB value per image.",message = FALSE}
...
---
**Can we cluster the outfits (images) based on the average R, G, B values?**

```{r echo = FALSE, message = FALSE}
## library for k-means clustering
library(factoextra)
## re format data. We deal only with the numerics info
df <- cluster_ducks[,2:4]
...
**What do we specify for $k$?**

```{r clusters}
library(factoextra)
## from two clusters to 6 (can we separate out the images?)
set.seed(4321) ## remove this when doing it your self
...
**But how many clusters are there?** it's a subjective choice

+ **Visual inspection** method
+ **Elbow** method
+ **Silhouette** method
+ **Gap** method
+ and the many others in your notes...


---
#### k-means clustering with `kmeans()`

**But how many clusters are there?** using the **elbow** method
...
### Key points to remember from module 1

**[slido.com](slido.com) code #biosci220: Your thoughts...**
...
+ **Hypothesis testing**
   ...
   + We can formulate our own tests! (e.g., randomizaion test)
...
<img src="https://pbs.twimg.com/media/EeMWb7QWsAIGftR?format=jpg&name=small", width="55%">
...
+ **Too many variables**
   ...
   + get rid of the junk and keep the information we need
  
--

+ **if you want to put things in 'boxes'**
    + k-means will probably do :-)
...
<img src="img/watty.jpg" width="50%">
