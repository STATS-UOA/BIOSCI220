...
b <- ggplot(stats_df,aes(x=Fs))+
   ...
   #              fill = "grey",
   #              alpha = 0.3,
   #              xlim = c(stats_df$Fs, 6),
   #              args = list(
   #                df1 = 2,df2 = 27
   #              )) + 
  # geom_area(data=data.frame(x = seq(stats_df$Fs,6,.1),
  #                           y  = df(seq(stats_df$Fs,6,.1),df1=2,df2=27)),
  #             aes(x=x,y=y), position = "identity") +
    transition_states(
        states=sims,
        transition_length = 2,
        state_length = 1
