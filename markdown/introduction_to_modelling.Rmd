...
Remember we are now dealing with a categorical variable so we use dummy variables (think back to the lecture video). Remember that the slopes associated with each prey value are in comparison to having `Coleoptera` as prey (this is included in the baseline). So we may conclude that given everything else in the model insects preyed on by `Diptera` have on average `r -1*round(coef(simple.mod)[2],2)` shorter stings than insects preyed on by `Coleoptera`.
