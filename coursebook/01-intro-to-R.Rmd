...
Following each step of this chapter carefully you are expected to install `R` and `RStudio` onto your computers as each week you will be expected to carry out a little programming! If you don't have your own computers then there are, of course, lab computers available that already have `R` and `RStudio` installed. If you choose to use these then you should still complete the exercises below to familiarise yourself with the software. Another option available is the use of [RStudio Cloud](https://rstudio.cloud/); here, everything is run in a web browser (on a remote server) and doesn't require you to download the software onto your personal computer. As a student at the University of Auckland you will already have an account associated with your UPI. 
...
paua <- read_csv("https://raw.githubusercontent.com/STATS-UOA/BIOSCI220/master/data/paua.csv?token=ADCWN3XSWTG6ACKEYZPXFFLADHQ22")
```
