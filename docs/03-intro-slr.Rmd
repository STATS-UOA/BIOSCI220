...
+ List the aims and write out the appropriate null and alternative hypothesis using statistical notation for a
    + one-sample t-test
    + two-sample t-test (independent and dependent)
    + randomization test
    + one-way Analysis of Variance (ANOVA)
+ Write R code to carry out a randomization test
+ Use the `lm()` function to carry out a
...
a <- ggplot(paua,aes(x = Species, y = Length)) + 
  ...
  theme(legend.position = "none") +
  geom_text(data = means, aes(x = Species, y = means + 25, label = paste0("Species averege = ",round(means,3)), color = Species))
a 
...
**Question:** Do we believe that the average length of P$\overline{\text{a}}$ua shells 
